...
def denoise_stack(input_path: Path, output_path: Path, model: SUPPORT,
    ...
    """Run inference on a single TIFF stack. Returns the output path.

    Length semantics:
      - ``include_edges='none'``: output has ``T - 2*pad`` frames,
        where ``pad = (model.in_channels - 1) // 2``. The first and
        last ``pad`` frames have no centred patch and are dropped.
      - ``include_edges='repeat'`` / ``'mirror'``: input is temporally
        padded by ``pad`` frames at each end with repeated or mirrored
        copies, so the output has ``T`` frames (matches input length).
    """
    ...
    if stack.ndim == 2:
        stack = stack[None, ...]
    if stack.ndim != 3:
        raise ValueError(f"expected 2- or 3-D TIFF; got shape {stack.shape}")
    ...
    # Hoist the device check out of the per-batch loop
    ...
    with torch.no_grad():
        for noisy, _, coords in tqdm(loader, desc="denoise"):
            ...
            for bi in range(noisy.size(0)):
                # Patch placement coordinates from the stitching dataset.
                # Each name is a torch tensor of length batch_size.
    ...
    # Crop edges (both code paths converge here):
    #   include_edges != 'none': we padded by `pad`, so `[pad:-pad]`
    #     returns the original length T_orig
    #   include_edges == 'none': frames [0, pad) and [T-pad, T) are
    #     zeros (no centred patch), so we drop them — output length
    #     is T_orig - 2*pad
    # When pad == 0 (2D mode, in_channels=1), no cropping is needed.
    if pad > 0:
