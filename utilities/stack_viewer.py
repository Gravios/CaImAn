#!/usr/bin/env python3
"""
stack_viewer.py — lightweight viewer for TIFF and MSR stacks
============================================================
Opens arbitrarily large 2-D-per-frame stacks (.tif / .tiff / .msr) without
loading the whole file into RAM. Each frame is read on demand through a
persistent ``StackReader`` handle; a small LRU cache keeps recently-visited
frames warm. Format dispatch is by extension.

Usage
-----
    python utilities/stack_viewer.py                       # file-open dialog
    python utilities/stack_viewer.py recording.tif
    python utilities/stack_viewer.py recording.msr
    python utilities/stack_viewer.py recording.tif 500     # open at frame 500
...
Adding a new format
-------------------
    Add a backend to ``caiman/utils/stack_io.py`` (mirror ``_TiffStackBackend``
    / ``_MsrStackBackend``) and register it in ``StackReader.__init__``.
...
class FrameCache:
    def __init__(self, reader: StackReader, maxsize: int = CACHE_FRAMES):
        self._reader  = reader
    ...
    def get(self, idx: int) -> np.ndarray:
        with self._lock:
            ...
            frame = self._reader.read_frame(idx).astype(np.float32)
...
class StackViewer:
    ...
    def __init__(self, root: tk.Tk, path: str, start_frame: int = 0):
        ...
        try:
            self.reader = StackReader(self.path)
        except (ValueError, OSError) as exc:
            messagebox.showerror("Error", f"Cannot open stack:\n{exc}")
            root.destroy()
            return
    ...
    def _build_ui(self):
        self.root.title(f"Stack Viewer — {self.path.name}")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Lightweight stack viewer for TIFF and MSR files",
    )
    parser.add_argument("stack", nargs="?",
                        help="Stack file to open (.tif/.tiff/.msr)")
    parser.add_argument("frame", nargs="?", type=int, default=0,
                        help="Frame index to start at (default 0)")
    args = parser.parse_args()

    path = args.stack or _pick_file()
    if path is None:
    ...
    root = tk.Tk()
    StackViewer(root, path, start_frame=args.frame or 0)
    root.mainloop()
