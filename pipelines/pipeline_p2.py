...
# Now datsrc/expsrc are set — safe to import pipeline_qc and set _qc_dir
import pipeline_qc as _qc
_qc_dir = datsrc / expsrc
...
# logger.info("Cleaning up shared memory at /dev/shm")
# import glob, os
# for _f in glob.glob('/dev/shm/caiman_*'):
#     try: os.unlink(_f)
#     except OSError: pass

# ── 12. CNMF fit ──────────────────────────────────────────────────────────────
import glob as _glob
for _shm in _glob.glob('/dev/shm/psm_*'):
    try:
        os.unlink(_shm)
