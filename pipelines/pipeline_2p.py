...
try:
    ...
    _pipelines_dir = Path(_inspect.getfile(_inspect.currentframe())).resolve().parent
...
try:
    _params_path = _pipelines_dir / Path((Path(__file__).stem + ".json"))
except NameError:
    _params_path = _pipelines_dir / (Path(_inspect.getfile(_inspect.currentframe())).stem + ".json")
...
# ParamBag gives dot-access with auto-coercion of lists → tuples.
# Hoist the frequently-used leaves into plain locals for readability.
ADD_BASELINE        = _P.data.add_baseline

fr                  = _P.data.fr
decay_time          = _P.data.decay_time

max_shifts          = _P.motion_correction.max_shifts          # tuple
strides             = _P.motion_correction.strides             # tuple
overlaps            = _P.motion_correction.overlaps            # tuple
...
n_processes         = _P.cluster.n_processes
...
# idx_good = cnm2.estimates.idx_components
# idx_bad  = cnm2.estimates.idx_components_bad

# snr  = cnm2.estimates.SNR_comp
# rval = cnm2.estimates.r_values
# cnn  = cnm2.estimates.cnn_preds

# min_snr  = cnm2.params.get('quality', 'min_SNR')
# rval_thr = cnm2.params.get('quality', 'rval_thr')
# cnn_thr  = cnm2.params.get('quality', 'min_cnn_thr')
# cnn_low  = cnm2.params.get('quality', 'cnn_lowest')

# print(f"Thresholds: SNR>{min_snr}  rval>{rval_thr}  CNN>{cnn_thr}  cnn_lowest>{cnn_low}")
# print()

# # Count what's failing among the rejected components
# fail_snr  = np.sum(snr[idx_bad]  < min_snr)
# fail_rval = np.sum(rval[idx_bad] < rval_thr)
# fail_cnn  = np.sum(cnn[idx_bad]  < cnn_low)

# print(f"Rejected ({len(idx_bad)}) breakdown:")
# print(f"  failed SNR only  : {np.sum((snr[idx_bad] < min_snr) & (rval[idx_bad] >= rval_thr) & (cnn[idx_bad] >= cnn_low))}")
# print(f"  failed rval only : {np.sum((snr[idx_bad] >= min_snr) & (rval[idx_bad] < rval_thr) & (cnn[idx_bad] >= cnn_low))}")
# print(f"  failed CNN only  : {np.sum((snr[idx_bad] >= min_snr) & (rval[idx_bad] >= rval_thr) & (cnn[idx_bad] < cnn_low))}")
# print(f"  failed multiple  : {np.sum((snr[idx_bad] < min_snr).astype(int) + (rval[idx_bad] < rval_thr).astype(int) + (cnn[idx_bad] < cnn_low).astype(int) > 1)}")
# print()
# print(f"Rejected SNR  — median: {np.median(snr[idx_bad]):.2f}  min: {np.min(snr[idx_bad]):.2f}  max: {np.max(snr[idx_bad]):.2f}")
# print(f"Rejected rval — median: {np.median(rval[idx_bad]):.2f}  min: {np.min(rval[idx_bad]):.2f}  max: {np.max(rval[idx_bad]):.2f}")
# print(f"Rejected CNN  — median: {np.median(cnn[idx_bad]):.2f}  min: {np.min(cnn[idx_bad]):.2f}  max: {np.max(cnn[idx_bad]):.2f}")
