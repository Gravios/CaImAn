"""
caiman/utils/param_summary.py
==============================
Log a formatted parameter table from a pipeline ``ParamBag``.
...
def log_params(
    ...
    *,
    ...
    ----------
    P
        ``ParamBag`` produced by
        :func:`~caiman.utils.params_io.load_pipeline_params`.
    logger
        Logger to write to.  Defaults to ``logging.getLogger("caiman")``.
    session
        Session identifier derived from the pipeline script filename.
        Displayed in the SESSION block in place of ``session_id``.
    """
    if logger is None:
